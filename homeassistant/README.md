...
| `sensor.nibe_electrical_input` | `43141 / 1000 + 43084` | Compressor inverter power plus immersion heater — measured, no estimates. **Excludes** the circulation pumps, the control board and inverter conversion losses. Register 43141 counts in units of **10 W**, not W — NibePi ≥ 1.4.6 scales it; before that it read a tenth of the truth and COP came out ten times too good |
| `sensor.nibe_pump_power` | GP1 `7.6 + 52.4 × n^3.36`, GP2 `178 × n²` | **Modelled**, not metered — neither pump reports power, only commanded speed. Curves fitted on an F1155-12 with the compressor idle and each pump metered directly; four points each, residuals under 0.4 W. The exponents differ because the circuits do: cold glycol in a long ground loop runs laminar-to-transitional (square law, no offset), the heating circuit is turbulent (near-cubic on a 7.6 W electronics floor). **These constants are specific to one machine** — refit them for yours, or expect the pump term to be wrong by tens of percent. GP1 keeps its heating curve during hot water; the tank coil is more restrictive so the real draw is a little lower, worth under 0.1 % of input |
| `sensor.nibe_electrical_input_total` | measured + pumps + `standby_w` | `standby_w` is **0** and known to be wrong: ~15 W on a 3×400 V feed is ~0.02 A per phase, under what a clamp meter resolves, so it was never measured. Costs roughly 130 kWh/yr in the energy totals. One line to fix if a meter ever goes on the supply |
| `sensor.nibe_cop` | heat output ÷ electrical input | A *compressor* COP. Both sides measured, but flattered by everything the denominator omits |
| `sensor.nibe_cop_total` | heat output ÷ electrical input total | Carries the pumps, so it runs ~3–5 % below `nibe_cop` at part load. The more honest of the two, with the caveat that its pump term is fitted. Both unavailable below 0.3 kW output, where the ratio is noise |
| `sensor.nibe_electric_energy` | Riemann sum of electrical input **total** | Integrated from the total because the pumps run during compressor-off periods where `nibe_electrical_input` sits at a flat zero — those hours are real kWh that used to vanish. Integration platform, `method: left`, **no `unit_prefix`**. The integration builds its unit as `<prefix><source unit><time>` and the source is already kW, so kW × h is kWh as it stands; asking for `k` gives `kkWh` and a result a further 1000× too small — which the Energy dashboard then charts as a flat zero |
